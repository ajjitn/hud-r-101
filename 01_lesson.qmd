...
-   tidyverse - a collection of libraries that use the same syntax/grammar
-   ggplot2 - for making plots/graphs

## Exercise 1 (Make a plot)

<font color="#55b748">**Step 1:**</font> Submit `head(airquality)` to the console. We will use the airquality dataset in this exericise

<font color="#55b748">**Step 2:**</font> Type the following in your script and run the code:
```{r, eval = F, echo =T}
ggplot(data=airquality)
```

Then add the following to your code and run it again. What's different? 
```{r, eval = F, echo =T}
ggplot(data=airquality, 
        mapping = aes(x = Temp, y = Ozone))
```

Then add the following and run again:
```{r, eval = F, echo =T}
ggplot(data=airquality, 
        mapping = aes(x = Temp, y = Ozone)) +
        geom_point()
...
<font color="#55b748">**Step 2:**</font> Clean the zillow data to just the most expensive cites using code below:
...
<font color="#55b748">**Step 1:**</font> Read in Multifamily housing data fro HUD using this code:

```{r, eval = F, echo = T}
mf_properties = read_sf("https://services.arcgis.com/VTyQ9soqVukalItT/arcgis/rest/services/MULTIFAMILY_PROPERTIES_ASSISTED/FeatureServer/7/query?outFields=HH_INCOME,PCT_HISPANIC,PCT_BLACK&where=1%3D1&f=geojson", 
                        quiet= TRUE)

```

<
<font color="#55b748">**Step 3:**</font>: In ggplot, plot the mf_properties dataset, fill it in using the `PCT_HISPANIC` column, add labels, change the outline (`colour`). Instead of using geom_point or geom_bar using geom_sf.
