...
In general, `readr`, `readxl`, and `haven` will come with some variant of a `read_*` function for data import, and a variant of `write_*` (except for Excel and SAS!) for data export.
...
<font color="#55b748">**Step 1:**</font> Use `write_csv()` to write out `ncvhis` to a comma separated file (csv).
...
-   weird NA values (like `N/A` or `-100`)
...
Note that the `readxl` package only provides functionality to, as the name implies, read Excel files. If you need to write your data out to `.xls` or `.xlsx` formats - [don't](https://www.moderncsv.com/why-excel-sucks-and-modern-csv-is-awesome-at-least-for-csvs/)! We highly recommend saving small-medium sized data files as `.csv` files and larger files (over 2 GB) as parquet files. But if you you really need to, look into the `writexl` or `openxlsx` packages.
...
We use `read_sas` to read in SAS files. This works mostly identical to `read_stata`, and you can read in `.sas7bdat` + `.sas7bcat`, or zipped folders with sas files. Note that while the `write_sas()` function exists, it doesn't work and will be removed soon.
...
Sometimes when you save text files, the column types (ie numeric, date, time, text, etc.) get lost and won't be read back in upon reimporting. Or you have. a really large dataset (>2 GB) which can take forever to read in and write out of R.

In these cases you may want to save your files as a binary file. One binary file format we suggest is `.feather` files, which you can [read more about](https://arrow.apache.org/docs/python/feather.html). The main benefits of storing data in a binary format is:
...
If you have really large data (>5 GB), you may alwo want to consider `.parquet` files which are another binary file format with even more compression and space savings.
